...
#ifndef WLS_REFRESH_TIME
#    define WLS_REFRESH_TIME 150
#endif

static uint8_t wls_devs = 0xFF;
static bool wls_reset = false;
...
void wireless_devs_change_main(uint8_t old_devs, uint8_t new_devs, bool reset) {
    ...
    if ((wls_devs != new_devs) || reset) {
        ...
        *md_getp_indicator() = 0;
        wls_devs = new_devs;
        wls_reset = reset;
        if (!(wls_devs == DEVS_USB && wls_reset)) {
            last_matrix_activity_trigger();
            md_devs_change(wls_devs, wls_reset);
        }
    }
}
void wireless_devs_change(uint8_t old_devs, uint8_t new_devs, bool reset) {
    wireless_devs_change_main(old_devs, new_devs, reset);
    wireless_devs_change_kb(old_devs, new_devs, reset);
    wireless_devs_change_user(old_devs, new_devs, reset);
}

void wireless_devs_change_nocallback(uint8_t old_devs, uint8_t new_devs, bool reset) {
    wireless_devs_change_main(old_devs, new_devs, reset);
}
